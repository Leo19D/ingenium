...
@pytest.mark.unit
async def test_login_step1_returns_otp_step(client: AsyncClient, db_session: AsyncSession):
    """Login s ispravnim podacima vraća step=otp, ne tokene."""
...
async def test_login_wrong_password(client: AsyncClient, db_session: AsyncSession):
    await _make_verified_user(db_session, "leo@ingeniumtrade.hr", "lozinka123")
    resp = await client.post("/api/v1/auth/login", json={
        "email": "leo@ingeniumtrade.hr", "password": "pogresna",
    })
...
async def test_login_unverified_user(client: AsyncClient, db_session: AsyncSession):
    ...
    resp = await client.post("/api/v1/auth/login", json={
        "email": "neverified@ingeniumtrade.hr", "password": "lozinka123",
    })
    assert resp.status_code == 403
...
async def _full_login(client: AsyncClient, db_session: AsyncSession, email: str, password: str) -> dict:
    """Simulira 2-koračni login s mockanim OTP-om '123456'. Vraća TokenResponse dict."""
    with patch("app.api.v1.auth.generate_otp", return_value="123456"), \
         patch("app.api.v1.auth.send_email", new_callable=AsyncMock):
        await client.post("/api/v1/auth/login", json={"email": email, "password": password})
    resp = await client.post("/api/v1/auth/verify-otp", json={"email": email, "code": "123456"})
...
async def test_verify_otp_wrong_code(client: AsyncClient, db_session: AsyncSession):
    ...
    resp = await client.post("/api/v1/auth/verify-otp", json={
        "email": "leo@ingeniumtrade.hr", "code": "000000",
    })
    assert resp.status_code == 401


@pytest.mark.unit
async def test_verify_otp_no_pending_session(client: AsyncClient, db_session: AsyncSession):
    """verify-otp bez prethodnog logina mora vratiti 401."""
    await _make_verified_user(db_session, "leo@ingeniumtrade.hr", "lozinka123")
    resp = await client.post("/api/v1/auth/verify-otp", json={
        "email": "leo@ingeniumtrade.hr", "code": "123456",
    })
    assert resp.status_code == 401


@pytest.mark.unit
async def test_verify_otp_max_attempts_locks_out(client: AsyncClient, db_session: AsyncSession):
    """3 kriva pokušaja resetiraju OTP i vraćaju grešku."""
    await _make_verified_user(db_session, "leo@ingeniumtrade.hr", "lozinka123")
    with patch("app.api.v1.auth.generate_otp", return_value="123456"), \
         patch("app.api.v1.auth.send_email", new_callable=AsyncMock):
        await client.post("/api/v1/auth/login", json={
    ...
    for _ in range(3):
        await client.post("/api/v1/auth/verify-otp", json={
            "email": "leo@ingeniumtrade.hr", "code": "000000",
        })
    # 4. pokušaj s ispravnim kodom mora biti odbijen (OTP je obrisan)
    resp = await client.post("/api/v1/auth/verify-otp", json={
        "email": "leo@ingeniumtrade.hr", "code": "123456",
    })
    assert resp.status_code == 401
...
async def test_clients_accessible_with_token(client: AsyncClient, db_session: AsyncSession):
    await _make_verified_user(db_session, "leo@ingeniumtrade.hr", "lozinka123")
    tokens = await _full_login(client, db_session, "leo@ingeniumtrade.hr", "lozinka123")
    resp = await client.get("/api/v1/clients/", headers={"Authorization": f"Bearer {tokens['access_token']}"})
    assert resp.status_code == 200
